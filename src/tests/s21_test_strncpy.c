...
#define ACTUAL(x, y, z) s21_strncpy(x, y, z)
#define EXPECTED(x, y, z) strncpy(x, y, z)
#define SUITE_LABEL "suite_s21_strncpy"
#define SUITE_NAME *suite_s21_strncpy()
#define STRINGS strings_strncpy
...
START_TEST(test) {
  ...
  char str1[256] = {
      ...
      "over‐flowed and that unused bytes in the destination buffer."};
  char str2[256] = {
      "One valid (and intended) use of strncpy() is to copy a C  string  to  a "
      "fixed-length  buffer  while  ensuring both that the buffer is not "
      "over‐flowed and that unused bytes in the destination buffer."};
  ...
  // range, expected_result, actual_result); // print for debug
