...
#define ACTUAL(x, y, z) s21_strncmp(x, y, z)
#define EXPECTED(x, y, z) strncmp(x, y, z)

#define SUITE_LABEL "suite_s21_strncmp"
...
START_TEST(test) {
  ...
  ck_assert_int_eq(actualres > 0, expectedres > 0);

  // TEST 4
  char *f4 = " \n\0";
  char *s4 = "a\n\0";
  actualres = ACTUAL(f4, s4, 0);
  expectedres = EXPECTED(f4, s4, 0);
