...
#define ACTUAL(x, y) s21_strcspn(x, y)
#define EXPECTED(x, y) strcspn(x, y)
#define SUITE_LABEL "suite_s21_strcspn"
#define SUITE_NAME *suite_s21_strcspn()
#define STRINGS strings_strcspn
...
START_TEST(test) { 
	s21_size_t actual_result;
	s21_size_t expected_result;
	char *sym = " X1fghj";

	actual_result = ACTUAL(STRINGS[_i], sym);
	expected_result = EXPECTED(STRINGS[_i], sym);

    //printf("\n str:%s char:%s res_orig:%ld res_act:%ld \n", STRINGS[_i], sym, actual_result, expected_result); // print for debug
