...
void parcer(const char *str, const char *format, s21_size_t *index,
            va_list list);
void get_length(const char *format, s21_size_t *index, STRSPEC *spec);
int get_precision(const char *format, s21_size_t *index, STRSPEC *spec,
                  va_list list);
void get_width(const char *format, s21_size_t *index, STRSPEC *spec,
               va_list list);
...
void binary_to_octal_hex(const char *binary_buffer, char octal_hex_type,
                         char *octal_hex_buffer);
...
void multiply_by_number(const char *buffer, int number, int place,
                        char *result_buffer);
void multiplication_exp(const char *buffer, int number, int exp,
                        char *result_buffer);
void multiplication(const char *buffer1, const char *buffer2,
                    char *result_buffer);
void real_mantissa(const char *binary_buffer, char *result_buffer);
void dot_move(char *value_buffer, int step);
void multiplication_exp_negative(const char *buffer, int exp,
                                 char *result_buffer);
...
void output(const char *str, va_list list, STRSPEC *spec) {
  if (spec->type == 'c' || spec->type == 'd' || spec->type == 'i' ||
      spec->type == 'u' || spec->type == 'o' || spec->type == 'x' ||
...
void print_value(const char *str, STRSPEC *spec, char *value_buffer) {
    ...
    if (spec->space_symbol == '0') {
      ...
      if ((spec->type == 'o' || spec->type == 'x' || spec->type == 'X') &&
        ...
        if (spec->type == 'x' || spec->type == 'X')
          print_symbol(str, spec->type);
      }
    }
    while (spaces--) print_symbol(str, spec->space_symbol);
  }
  ...
  if ((spec->type == 'o' || spec->type == 'x' || spec->type == 'X') &&
      spec->hash && spec->space_symbol != '0') {
    print_symbol(str, '0');
    if (spec->type == 'x' || spec->type == 'X') print_symbol(str, spec->type);
  }

  if (spec->type == 's' && spec->dot == 1) {
    for (s21_size_t i = 0; i < spec->precision && value_buffer[i]; i++)
      print_symbol(str, value_buffer[i]);
  } else {
...
void trim_real(char *value_buffer, STRSPEC *spec) {
  s21_size_t length = s21_strlen(value_buffer),
             dot_index = dot_place(value_buffer);
...
s21_size_t binary_to_decimal_value(const char *binary_buffer, STRSPEC *spec) {
  ...
  for (int i = 0; i <= last_bit; i++) {
    value +=
        (binary_buffer[i] - '0') * (s21_size_t)s21_int_pow(binary_radix, i);
  }
...
void binary_to_octal_hex(const char *binary_buffer, char octal_hex_type,
  ...
  for (int i = 0; i <= last_bit; i++) {
    step_sum +=
        (binary_buffer[i] - '0') * (int)s21_int_pow(binary_radix, place);
    place++;
  ...
  step_count--;
  while (step_count && octal_hex_buffer[step_count] == '0')
    octal_hex_buffer[step_count--] = '\0';
...
void copy_buffer(const char *buffer, char *result_buffer) {
  ...
  for (int i = 0; i < length; i++) result_buffer[i] = buffer[i];
...
void multiplication(const char *buffer1, const char *buffer2,
  ...
  for (int i = 0; i < length2; i++) {
    char temp_buffer[max_buf_size] = {0},
         temp_buffer_result[max_buf_size] = {0};
